...
its = ImageType.where(name: ['original','thumb', 'promo'])
c1.image_types << [its]
new_image_c1 = Image.create!(attachable_id: c1.id, attachable_type: c1.class.name)
test_file_c1 = grab_test_file(new_image_c1.attachable_id)
# new_image_c1.recreate_versions!
new_image_c1.name.store!(test_file_c1)
c1.images << new_image_c1
c1.save!
...
# new_image_c2.recreate_versions!
...
new_image_t1.name.store!(test_file_t1)
...
new_image_t2.name.store!(test_file_t2)
...
new_image_t3.name.store!(test_file_t3)
